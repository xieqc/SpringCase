    ...
    public void service() throws IOException {
        while (true) {
            ...
            try{
                ...
                Handler handler = new Handler(socket, ++socketKey);
...
class Handler implements Runnable{
    private Integer socketKey;
    private Socket socket;
    public BufferedReader is;
    public PrintWriter os;
    public Handler(Socket socket, Integer socketKey) throws IOException {
        this.socket = socket;
    ...
    public void run(){
        try{
            String line;
            os.println(socketKey+" welcome logon."+MultiThreadServer.socketMap.size());	//向客户端输出该字符串
            os.flush();	//刷新输出流，使Client马上收到该字符串
            line = is.readLine();	//从标准输入读入一字符串
            while((line != null) && (!line.endsWith("bye"))){	//如果该字符串为 "bye"，则停止循环
                String [] str = line.split(":");
                //向目标输出信息
                Handler targetHandler = MultiThreadServer.socketMap.get(Integer.valueOf(str[0]));
                targetHandler.os.println("accpet:"+line);
                targetHandler.os.flush();
            ...
            os.flush();
            MultiThreadServer.socketMap.remove(socketKey);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
          try {
